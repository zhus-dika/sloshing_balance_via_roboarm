...
#define MAX_ATTEMPTS   100          /* сколько раз подряд подождать файл-флаг   */
...
/* --- геом./мат. параметры --------------------------------------- */
#define ZONE_VESSEL_LIQ   15      /* id зоны внутри сосуда          */
#define WATER_PHASE_IDX    1      /* "water" в списке фаз           */
#define VOF_THR         0.80      /* минимальная доля для учёта      */
#define RHO_W        1000.0       /* плотность воды, кг/м³           */
#define M_VESSEL        0.35      /* масса пустого сосуда, кг        */
/* собственный тензор инерции пустого сосуда в КС сосуда */
...
real cx,cy,cz, q[4], velocity[3], omega[3];
real t_prev = 0.0;
...
/* 2. Считываем pose.dat или демо-синус */
DEFINE_ADJUST(read_pose_from_simulink, domain)
{
    real t = CURRENT_TIME;
    if (fabs(t - t_prev) < 1e-12) return;   /* уже вызывались на этом шаге   */
    t_prev = t;

    if (I_AM_NODE_ZERO_P) {

        /* ----------- несколько попыток найти и обработать pose.ok --------- */
            ...
            if (flag_exists(FLAG_IN)) {
                ...
                if (fp) {
                    ...
                    if (ok == 13)
                        ...
                        Message("⚠️  Pose file corrupted (read %d/13)\n", ok);

                } else {
                    Message("⚠️  Can't open %s\n", POSE_IN);
                }

                /* файл-флаг обработан — удаляем и выходим из цикла */
                remove(FLAG_IN);
                break;
            }

            /* флага нет — короткий сон и новая попытка */
            usleep(WAIT_US);
        }
    }

    /* ---------- рассылаем принятую позу по всем ранкам/узлам ---------- */
...
DEFINE_EXECUTE_AT_END(write_props_to_simulink)
{
    ...
    real m_liq=0, M1[3]={0}, I[6]={0}, vol_spill=0;
    ...
    thread_loop_c(t,d){
        ...
        begin_c_loop_all(c,t){
            real vf=C_VOF(c,tw); if(vf<VOF_THR) continue;
            ...
            m_liq += dm;
            M1[0]+=dm*xc[0]; M1[1]+=dm*xc[1]; M1[2]+=dm*xc[2];

            /* тензор инерции w.r.t origin */
            I[0]+=dm*(xc[1]*xc[1]+xc[2]*xc[2]);          /* Ixx */
            I[1]+=dm*(xc[0]*xc[0]+xc[2]*xc[2]);          /* Iyy */
            I[2]+=dm*(xc[0]*xc[0]+xc[1]*xc[1]);          /* Izz */
            I[3]-=dm*xc[0]*xc[1];                        /* Ixy */
            I[4]-=dm*xc[1]*xc[2];                        /* Iyz */
            I[5]-=dm*xc[0]*xc[2];                        /* Ixz */

            if(!inside) vol_spill += vol;
        }
        end_c_loop_all(c,t)
    }
    /* MPI-суммы */
    ...
    for(int i=0;i<6;i++) I[i]=PRF_GRSUM1(I[i]);
    ...
    /* центр масс жидкости */
    real com_l[3]={0,0,0};
    if(m_liq>0){ com_l[0]=M1[0]/m_liq; com_l[1]=M1[1]/m_liq; com_l[2]=M1[2]/m_liq; }
    ...
    real I_tot[6]={
        I_VES_X+I[0], I_VES_Y+I[1], I_VES_Z+I[2],
        I[3], I[4], I[5]
    };
    ...
    if(fp){
        ...
        Message("✅ feed.dat written (t=%.3e, m_liq=%.3f kg, spill=%.3e m³)\n",
