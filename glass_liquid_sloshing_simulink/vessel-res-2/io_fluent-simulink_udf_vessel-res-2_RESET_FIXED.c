...
#define WAIT_US        2000000   /* 2 s; total pose wait timeout ~= 300 s */
...
#define DEFAULT_RESET_UDF_FLAG "/home/dika/Documents/sloshing_balance_via_roboarm/glass_liquid_sloshing_simulink/vessel-res-2/reset/reset_udf.ok"
#define DEFAULT_RESET_CASE_FLAG "/home/dika/Documents/sloshing_balance_via_roboarm/glass_liquid_sloshing_simulink/vessel-res-2/reset/reset_case.ok"

/* ENV override:
 * UDF_FLAG_IN, UDF_POSE_IN, UDF_FLAG_OUT, UDF_FEED_OUT,
 * UDF_RESET_UDF_FLAG, UDF_RESET_CASE_FLAG
 */
...
static real spill_in_step    = 0.0;   /* положительное Δ объёма вне зоны сосуда за текущий output */
...
static int  pose_inited = 0;  /* C0/Q0 зафиксированы из первого pose нового эпизода */
static int  pose_valid  = 0;  /* разрешает применять pose/velocity к Dynamic Mesh */
...
/* Полный сброс UDF-состояния между эпизодами.
 * Вызывается на всех compute-процессах после broadcast reset_udf.ok.
 * Следующий валидный pose заново задаст CX/CY/CZ/Q и базу C0/Q0.
 */
static void reset_udf_state(void)
{
    step_cnt_in  = 0;
    step_cnt_out = 0;
    t_prev       = -1.0;
    ...
    pose[3] = 1.0;  /* quaternion w in pose = [r(3), q(wxyz), v(3), w(3)] */
...
{
    /* После reset и до первого нового pose не применять состояние прошлого эпизода. */
    if (!pose_valid) {
        NV_S(vel,   =, 0.0);
        NV_S(omega, =, 0.0);
        return;
...
{
    const char *flag_in =
        resolve_path_env("UDF_FLAG_IN", DEFAULT_FLAG_IN);
    const char *pose_in =
        resolve_path_env("UDF_POSE_IN", DEFAULT_POSE_IN);
    const char *reset_udf_flag =
        resolve_path_env("UDF_RESET_UDF_FLAG", DEFAULT_RESET_UDF_FLAG);
    const char *reset_case_flag =
        resolve_path_env("UDF_RESET_CASE_FLAG", DEFAULT_RESET_CASE_FLAG);

    int do_reset = 0;
    int case_reset_pending = 0;
    int got_pose = 0;

    /* =========================================================
     * 0. RESET ИМЕЕТ АБСОЛЮТНЫЙ ПРИОРИТЕТ.
     * Проверяем ДО CURRENT_TIME и ДО SKIP, чтобы reset не мог
     * застрять из-за duplicate-time guard или decimation.
     * ========================================================= */
    if (I_AM_NODE_ZERO_P) {
        do_reset = flag_exists(reset_udf_flag) ? 1 : 0;
    }
    PRF_BCAST_INT(&do_reset, 1, 0);

    if (do_reset) {
        /* Сбросить статическое состояние на всех процессах. */
        reset_udf_state();

        /* reset_udf.ok является acknowledgement:
         * удаляем его только ПОСЛЕ выполнения reset_udf_state()
         * на node zero и после broadcast команды reset.
         */
        if (I_AM_NODE_ZERO_P) {
            /* Отбросить незабранный pose прошлого эпизода ДО reset ACK.
             * pose.dat может остаться: без pose.ok он не будет прочитан,
             * а kinovaResetFcn удалит dat-файлы после обоих reset ACK.
             */
            if (flag_exists(flag_in)) {
                if (remove(flag_in) != 0 && errno != ENOENT) {
                    Message("Warning: can't remove stale %s (errno=%d)\n",
                            flag_in, errno);
                }
            }

            if (remove(reset_udf_flag) == 0) {
                Message("reset_udf.ok processed: UDF state reset.\n");
            } else if (errno != ENOENT) {
                Message("Warning: can't remove %s (errno=%d)\n",
                        reset_udf_flag, errno);
            }
        }
        return;
    }

    /* Пока SCM ещё держит reset_case.ok, не входить в ожидание pose.
     * Это позволяет Fluent закончить/reload golden case без deadlock.
     * Файл reset_case.ok удаляет только SCM, UDF его НЕ трогает.
     */
    if (I_AM_NODE_ZERO_P) {
        case_reset_pending = flag_exists(reset_case_flag) ? 1 : 0;
    }
    PRF_BCAST_INT(&case_reset_pending, 1, 0);
    if (case_reset_pending) return;

    /* Один вызов на новый physical time. */
    {
        real t = CURRENT_TIME;
    ...
    /* Первый pose после reset читаем НЕМЕДЛЕННО.
     *
     * После первого pose step_cnt_in остаётся 0. Поэтому:
     *   first pose -> 20 CFD steps -> first feed
     *              -> next CFD step reads next pose
     *
     * Это сохраняет правильную фазировку handshake и не создаёт
     * deadlock "Fluent ждёт pose / Simulink ждёт feed" на 20-м шаге.
     */
    if (pose_valid) {
    ...
    /* =========================================================
     * 1. Ждать pose.ok, но ВНУТРИ polling снова проверять reset.
     * Это критично: localResetFcn должен иметь возможность
     * вытащить Fluent из ожидания pose.ok.
     * ========================================================= */
    if (I_AM_NODE_ZERO_P) {
        int k;

        for (k = 0; k < MAX_ATTEMPTS; ++k) {

            /* reset_udf.ok приоритетнее любого старого pose.ok */
            if (flag_exists(reset_udf_flag)) {
                do_reset = 1;
                break;
            }

            /* Если SCM начал/продолжает reload case — уступить управление. */
            if (flag_exists(reset_case_flag)) {
                case_reset_pending = 1;
                break;
            }
            ...
            if (flag_exists(flag_in)) {
                ...
                if (fp) {
                    ...
                    if (ok == 13) {
                        ...
                        Message("Warning: pose.dat corrupted (%d/13); "
                                "pose rejected.\n", ok);
                    }

                    /* pose.ok снимаем после завершения попытки чтения.
                     * Если pose повреждён, feed не будет опубликован:
                     * Simulink завершит эпизод по своему timeout.
                     */
                    if (remove(flag_in) != 0 && errno != ENOENT) {
                        Message("Warning: can't remove %s (errno=%d)\n",
                                flag_in, errno);
                    ...
                    Message("Warning: cannot open %s; pose rejected.\n",
                            pose_in);
        ...
        if (!got_pose && !do_reset && !case_reset_pending &&
            k >= MAX_ATTEMPTS) {
            Message("ERROR: timeout waiting for pose.ok (~%.1f s).\n",
                    ((real)MAX_ATTEMPTS * (real)WAIT_US) / 1.0e6);
        }
    }

    /* Reset мог появиться уже ПОСЛЕ входа в polling. */
    PRF_BCAST_INT(&do_reset, 1, 0);
    ...
    if (do_reset) {
        ...
        if (I_AM_NODE_ZERO_P) {
            ...
            if (remove(reset_udf_flag) == 0) {
                Message("reset_udf.ok processed while waiting pose: "
                        "UDF state reset.\n");
            } else if (errno != ENOENT) {
                Message("Warning: can't remove %s (errno=%d)\n",
                        reset_udf_flag, errno);
    ...
    PRF_BCAST_INT(&got_pose, 1, 0);

    if (!got_pose) {
        /* Не продолжать CFD/output со старой кинематикой.
         * Simulink timeout затем завершит эпизод и вызовет ResetFcn.
         */
        pose_valid = 0;
        return;
    }
    ...
    /* Первый валидный pose каждого эпизода задаёт новую базу COM_loc. */
    if (!pose_inited) {
        C0[0] = CX;
        C0[1] = CY;
        C0[2] = CZ;
        ...
        if (I_AM_NODE_ZERO_P) {
            Message("First pose accepted after reset: C0/Q0 reinitialized.\n");
        }
    }
}
...
{
    const char *flag_out =
        resolve_path_env("UDF_FLAG_OUT", DEFAULT_FLAG_OUT);
    const char *feed_out =
        resolve_path_env("UDF_FEED_OUT", DEFAULT_FEED_OUT);

    /* После reset не публиковать stale feed до первого нового pose/C0/Q0. */
    if (!pose_valid || !pose_inited) return;
